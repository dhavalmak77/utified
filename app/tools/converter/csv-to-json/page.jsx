...
export default function Base64() {
		...
		Papa.parse(csvText, {
			...
			complete: function (results) {
				...
				if (typeof results.data === 'string') {
					parsedObject = eval('(' + results.data + ')');
				} else {
					parsedObject = results.data;
				}

				const jsonString = JSON.stringify(parsedObject, null, space);
				setDecodeInput(jsonString);

				addToHistory({ encodeInput, decodeInput: jsonString });
			},
		});
	};
	...
	const handleDownload = useCallback(
		(typeIdentity) => {
			const decodeInput = typeIdentity === __CSV ? encodeInput : decodeInput;
			...
			const blob = new Blob([decodeInput], { type: mimeType });
	...
	return (
		<PageWrapper
			...
			description='Easily convert your JSON data with JSON.'
			aside={true}
		>
			{/* Common Card */}
