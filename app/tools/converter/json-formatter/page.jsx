	...
	const handleConversion = () => {
		...
		try {
			...
			if (typeof encodeInput === 'string') {
				parsedObject = JSON.parse(encodeInput);
			} else {
			...
			addToHistory({ encodeInput: encodeInput, decodeInput: jsonString });
		} catch (error) {
			setDecodeInput('');
			...
			if (error instanceof SyntaxError) {
				...
				console.log(`Failed to fetch file: ${error.response.status} - ${error.response.data}`);
				setError(`${error.response.status} - ${error.response.data}`);
			} else if (error.request) {
				console.log('Failed to fetch file: No response from server.');
