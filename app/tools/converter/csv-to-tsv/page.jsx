...
export default function Base64() {
	...
	// Handler for CSV to TSV conversion
	const handleConversion = (csvString = false) => {
		...
		Papa.parse(csvText, {
			...
			complete: function (results) {
				...
				const tsvHeader = columns.join('\t');
				const tsvRows = results.data.map((row) => columns.map((col) => (row[col] != null ? row[col] : '')).join('\t'));

				// Combine header and rows into a single TSV string
				const tsvText = [tsvHeader, ...tsvRows].join('\n');

				setDecodeInput(tsvText);

				// Adding history for TSV conversion
				addToHistory({ encodeInput, decodeInput: tsvText });
			},
		});
	};
	...
	const handleDownload = useCallback(
		(typeIdentity) => {
			...
			const mimeType = typeIdentity === __CSV ? 'text/csv' : 'text/tab-separated-values;charset=utf-8';
			const fileExtension = typeIdentity === __CSV ? 'csv' : 'tsv';
	...
	return (
		<PageWrapper
			...
			description='Easily convert your CSV data with TSV.'
			aside={true}
		>
			{/* Common Card */}
