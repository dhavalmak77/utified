...
const convertXMLToHTML = async (xmlString) => {
	try {
		const result = await parseStringPromise(xmlString);

		// A function to convert JS object to HTML
		const objectToHTML = (obj) => {
			if (Array.isArray(obj)) {
				return obj.map((item) => objectToHTML(item)).join('');
			}
			if (typeof obj !== 'object') return obj;

			return Object.entries(obj)
				.map(([key, value]) => {
					const children = objectToHTML(value);
					return `<${key}>${children}</${key}>`;
				})
				.join('');
		};

		const htmlString = objectToHTML(result);
		return htmlString.replace(/<([^>]+)>(\s+)?<\/\1>/g, ''); // Clean up empty tags
	} catch (error) {
...
export default function Base64() {
	...
	const [qrValues, setQrValues] = useState({ [__INPUT]: '', [__OUTPUT]: '' });
	const [copyStatus, setCopyStatus] = useState(initialCopyStatus);
	const [settings, setSettings] = useState([__INPUT]);
	...
	useEffect(() => {
		if (showQRCode.includes(__INPUT) || showQRCode.includes(__OUTPUT)) {
			const qrCodeId = setTimeout(() => {
				setQrValues({
					[__INPUT]: encodeInput,
					[__OUTPUT]: decodeInput,
				});
			}, 750);
	...
	const handleConversion = async () => {
		const html = await convertXMLToHTML(encodeInput);
		setDecodeInput(html);
	};
	...
	const handleCopy = useCallback(
		async (conversionType) => {
			try {
				const textToCopy = conversionType === __INPUT ? encodeInput : decodeInput;
	...
	const handleDownload = useCallback(
		(typeIdentity) => {
			const decodeInput = typeIdentity === __INPUT ? encodeInput : decodeInput;
	...
	return (
		<PageWrapper
			title='XML to HTML Converter'
			description='Convert XML data to HTML easily.'
			aside={true}
		>
			{/* Common Card */}
			<div className='h-auto w-full bg-slate-100 border border-[#ced4da] flex rounded-md divide-x divide-[#ced4da]'>
				...
				<div className='p-4 rounded-md rounded-r-none flex-1 flex flex-col gap-4 w-full'>
					<div className='flex w-full gap-0 items-end'>
						...
						<Textarea
							label='XML'
							description='Enter XML to convert'
							size='md'
							rows={10}
							w={'100%'}
							className={showQRCode.includes(__INPUT) && qrValues[__INPUT].length ? 'active-qr' : ''}
							placeholder='Enter XML or upload a file'
							radius='md'
							value={encodeInput}
							error=''
							onChange={(e) => {
						...
						/>
						{showQRCode.includes(__INPUT) && qrValues[__INPUT] && (
							<div className='border-t border-r border-b border-[#d9d9d9] rounded-tr-md rounded-br-md p-1.5'>
								<QRCodeSVG
									size={250}
									value={qrValues[__INPUT].slice(0, 1000)}
									level='L'
									bordered='false'
									status={qrValues[__INPUT].length > 1000 ? 'length-error' : 'active'}
								/>
							</div>
						)}
					</div>
					...
					<Group justify='space-between'>
						<Group>
							<Button
								variant='filled'
								// className={cn(!autoSync[__INPUT] && 'rounded-md py-2 border-blue-500 text-blue-500 bg-transparent hover:bg-blue-500 hover:text-white')}
								disabled={autoSync}
						...
						>
							<Tooltip label='Settings'>
								<Button
									onClick={() => handleSettings(__INPUT)}
									unstyled
									c={settings.includes(__INPUT) ? 'blue' : ''}
									p={10}
								>
									{settings.includes(__INPUT) ? <TbSettingsMinus size={16} /> : <TbSettingsPlus size={16} />}
								</Button>
							</Tooltip>
							<Tooltip label={copyStatus.type !== __INPUT ? 'Copy' : copyStatus.isSuccess ? 'Copied' : copyStatus.hasError ? 'Error copying' : 'Copy'}>
								<Button
									variant={hoveredCopy1 ? 'filled' : 'default'}
									onClick={() => handleCopy(__INPUT)}
									...
									ref={refCopy1}
								>
									{copyStatus.type !== __INPUT ? <TbCopy size={16} /> : <TbCopyCheck size={16} />}
								</Button>
							</Tooltip>
							<Tooltip label='Download'>
								<Button
									variant={hoveredDownload1 ? 'filled' : 'default'}
									onClick={() => handleDownload(__INPUT)}
									// className={cn('bg-blue-500 text-white border border-blue-500 transition-colors', 'hover:bg-white hover:text-blue-500 hover:border-blue-500', { 'opacity-50 cursor-not-allowed': !encodeInput })}
									disabled={!encodeInput}
									px={10}
									ref={refDownload1}
					...
					<Group
						...
						hidden={!settings.includes(__INPUT)}
					>
						<Group
							spacing={0}
							gap={10}
						...
						>
							...
							<Tooltip
								label={showQRCode.includes(__INPUT) ? 'QR Code is active' : 'QR Code'}
								withArrow
								arrowSize={8}
							>
								<Button
									variant={hoveredQr1 || showQRCode.includes(__INPUT) ? 'filled' : 'default'}
									// variant={showQRCode.includes(__INPUT) ? 'filled' : 'default'}
									onClick={() => handleQRCode(__INPUT)}
									className='text-black border-none rounded-md bg-transparent hover:text-blue-500 shadow-none'
									leftSection={showQRCode.includes(__INPUT) ? <TbCircleCheck size={18} /> : <TbQrcode size={18} />}
									ref={refQr1}
								>
									QR Code
								</Button>
				...
				<div className='p-4 rounded-md rounded-r-none flex-1 flex flex-col gap-4'>
					<Group className='flex flex-col justify-center items-center'>
						<div className='flex w-full gap-0 items-end'>
							...
							<Textarea
								...
								rows={10}
								w={'100%'}
								className={showQRCode.includes(__OUTPUT) && qrValues[__OUTPUT].length ? 'active-qr' : ''}
								placeholder='Converted HTML will appear here'
								radius='md'
								value={decodeInput}
								error=''
								onChange={(e) => {
									if (autoSync[__OUTPUT]) {
										handleConversion(__OUTPUT, e.target.value);
									}
							...
							/>
							{showQRCode.includes(__OUTPUT) && qrValues[__OUTPUT] && (
								<div className='border-t border-r border-b border-[#d9d9d9] rounded-tr-md rounded-br-md p-1.5'>
									<QRCodeSVG
										size={250}
										value={qrValues[__OUTPUT].slice(0, 1000)}
										level='L'
										bordered='false'
										status={qrValues[__OUTPUT].length > 1000 ? 'length-error' : 'active'}
									/>
								</div>
							)}
						</div>
					...
					<Group justify='space-between'>
						...
						>
							<Tooltip label='Settings'>
								<Button
									onClick={() => handleSettings(__OUTPUT)}
									unstyled
									c={settings.includes(__OUTPUT) ? 'blue' : ''}
									p={10}
								>
									{settings.includes(__OUTPUT) ? <TbSettingsMinus size={16} /> : <TbSettingsPlus size={16} />}
								</Button>
							</Tooltip>
							<Tooltip label={copyStatus.type !== __OUTPUT ? 'Copy' : copyStatus.isSuccess ? 'Copied' : copyStatus.hasError ? 'Error copying' : 'Copy'}>
								<Button
									variant={hoveredCopy2 ? 'filled' : 'default'}
									onClick={() => handleCopy(__OUTPUT)}
									...
									ref={refCopy2}
								>
									{copyStatus.type !== __OUTPUT ? <TbCopy size={16} /> : <TbCopyCheck size={16} />}
								</Button>
							</Tooltip>
							<Tooltip label='Download'>
								<Button
									variant={hoveredDownload2 ? 'filled' : 'default'}
									onClick={() => handleDownload(__OUTPUT)}
									// className={cn('bg-blue-500 text-white border border-blue-500 transition-colors', 'hover:bg-white hover:text-blue-500 hover:border-blue-500', { 'opacity-50 cursor-not-allowed': !decodeInput })}
									disabled={!decodeInput}
									px={10}
									ref={refDownload2}
					...
					<Group
						...
						gap={10}
						justify='space-between'
						hidden={!settings.includes(__OUTPUT)}
					>
						<Group
							spacing={0}
							gap={10}
						...
						>
							<Tooltip
								label={showQRCode.includes(__OUTPUT) ? 'QR Code is active' : 'QR Code'}
								withArrow
								arrowSize={8}
							>
								<Button
									variant={hoveredQr2 || showQRCode.includes(__OUTPUT) ? 'filled' : 'default'}
									// variant={showQRCode.includes(__OUTPUT) ? 'filled' : 'default'}
									onClick={() => handleQRCode(__OUTPUT)}
									className='text-black border-none rounded-md bg-transparent hover:text-blue-500 shadow-none'
									leftSection={showQRCode.includes(__OUTPUT) ? <TbCircleCheck size={18} /> : <TbQrcode size={18} />}
									ref={refQr2}
								>
									QR Code
								</Button>
