...
const currentTool = 'joaat';
const rows = 5;

export default function Joaat() {
	...
	const { inputValue, setInputValue, outputValue, setOutputValue, settings, toggleSettings, autoSync, toggleAutoSync, qrValues, showQRCode, toggleQRCode, loading, setLoading, addToHistory, undo, redo, canUndo, canRedo, error, addError, clearError } =
		...
		const generateHash = (data) => {
			...
			debounce(
				...
				async () => {
					try {
						const resultHash = await generatePhpHash(`joaat`, data);
	...
	return (
		<PageWrapper
			title='Joaat Hash Generator'
			description='Generate Joaat hash.'
			aside={true}
		>
			<ToolBoxWrapper>
				{/* First column */}
