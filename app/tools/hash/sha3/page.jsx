...
import { ActionIcon, Box, Button, Checkbox, FileButton, Group, Select, Text, Textarea, Tooltip } from '@mantine/core';
...
const sha3Sizes = [
	{ label: '224', value: 'sha3_224' },
	{ label: '256', value: 'sha3_256' },
	{ label: '384', value: 'sha3_384' },
	{ label: '512', value: 'sha3_512' },
];
...
export default function Sha3() {
	...
	const [settings, setSettings] = useState([INPUT, HASH]);
	const [showQRCode, setShowQRCode] = useState([]);
	const [autoSync, setAutoSync] = useState({ [INPUT]: false, [HASH]: false });
	const [history, setHistory] = useState([]);
	const [sha3Size, setSha3Size] = useState('sha3_256');
	...
	const generateHash = (data) => {
		try {
			const resultHash = jsSha3[sha3Size](data);
			setHash(resultHash);
			setError('');
	...
	return (
		<PageWrapper
			title='sha3 Hash Generator'
			description='Generate sha3 hash.'
			aside={true}
		>
			{/* Common Card */}
					...
					>
						...
						>
							<Select
								...
								onChange={(value) => setSha3Size(value)}
								allowDeselect={false}
								searchable
								leftSection={<Text px={10} c='black'>Size</Text>}
								leftSectionWidth={50}
								w={108}
							/>
						</Group>
