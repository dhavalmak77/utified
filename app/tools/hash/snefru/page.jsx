...
// Function to create a RIPEMD hash of a given message and length
function calculateRipemdHash(message, length) {
...
export default function SnefruPage() {
	...
	const generateHash = (data) => {
		try {
			let resultHash = new Snefru({ rounds: 8, length: 256 });
			resultHash.update(data);
			resultHash = toHex(resultHash.finalize());
			setHash(resultHash);
			setError('');
	...
	return (
		<PageWrapper
			title='Snefru Hash Generator'
			description='Generate Snefru hash.'
			aside={true}
		>
			{/* Common Card */}
